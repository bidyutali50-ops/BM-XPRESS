...
function TiltCard({ index, title, description }: { index: number; title: string; description: string }) {
      ...
      <div className="container">
        ...
        <div className="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((service, i) => (
            <TiltCard key={service.title} index={i} title={service.title} description={service.description} />
          ))}
        </div>
      </div>
    </section>
