...
function DottedGlobe() {
  ...
  return (
    <points geometry={geometry}>
      <pointsMaterial size={0.022} color="#8B5CF6" sizeAttenuation transparent opacity={0.55} />
    </points>
  );
}
...
function Routes() {
  ...
  return (
    <group>
      {arcs.map((arc, i) => (
        <Line key={i} points={arc} color="#7C5CFC" lineWidth={1.4} transparent opacity={0.55} />
      ))}
      {hubVectors.map((v, i) => (
        <mesh key={i} position={v}>
          <sphereGeometry args={[i === 0 ? 0.045 : 0.03, 12, 12]} />
          <meshBasicMaterial color={i === 0 ? "#2A2140" : "#7C5CFC"} />
        </mesh>
      ))}
    </group>
  );
...
function OrbitingMarker() {
  ...
  return (
    <mesh ref={ref}>
      ...
      <meshStandardMaterial color="#7C5CFC" emissive="#7C5CFC" emissiveIntensity={1.4} />
    </mesh>
  );
}
  ...
  return (
    <group ref={groupRef}>
      <Sphere args={[RADIUS - 0.02, 48, 48]}>
        <meshBasicMaterial color="#F3EFFF" transparent opacity={0.75} />
      </Sphere>
      <DottedGlobe />
      <Routes />
      <OrbitingMarker />
